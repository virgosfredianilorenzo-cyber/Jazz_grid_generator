...
function duplicateSection(si){snapshotUndo();const c=JSON.parse(JSON.stringify(chartData.sections[si]));c.label+=' (bis)';c.id=_newSectionId();chartData.sections.splice(si+1,0,c);render();}
...
function exportJSON(){syncMetaToData();const clean=JSON.parse(JSON.stringify(chartData));clean.sections.forEach(s=>s.measures.forEach(m=>m.chords.forEach(c=>{delete c._originalSymbol;})));const blob=new Blob([JSON.stringify(clean,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=(chartData.title||'chart')+'.json';a.click();_isDirty=false;}
document.getElementById('import-json').addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>{try{chartData=JSON.parse(ev.target.result);_ensureSectionIds(chartData);originalKey=chartData.key;currentSemitoneOffset=0;document.getElementById('semitone-display').textContent='0';document.getElementById('transpose-key-select').value='';document.getElementById('dropzone').style.display='none';document.getElementById('chart-editor').style.display='block';_isDirty=false;render();}catch{alert(t('alertBadJSON'));}};r.readAsText(file);});
document.getElementById('file-input').addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;try{const xmlStr=await loadFileAsXML(file);chartData=parseMusicXML(xmlStr);_ensureSectionIds(chartData);originalKey=chartData.key;currentSemitoneOffset=0;document.getElementById('semitone-display').textContent='0';document.getElementById('transpose-key-select').value='';autoAnnotateOnImport();document.getElementById('dropzone').style.display='none';document.getElementById('chart-editor').style.display='block';_isDirty=false;render();}catch(err){console.error(err);alert(t('alertBadXML'));}e.target.value='';});
const dz=document.getElementById('dropzone');
dz.addEventListener('dragover',e=>{e.preventDefault();dz.classList.add('drag-over');});
dz.addEventListener('dragleave',()=>dz.classList.remove('drag-over'));
dz.addEventListener('drop',async e=>{e.preventDefault();dz.classList.remove('drag-over');const file=e.dataTransfer.files[0];if(!file)return;try{const xmlStr=await loadFileAsXML(file);chartData=parseMusicXML(xmlStr);_ensureSectionIds(chartData);originalKey=chartData.key;currentSemitoneOffset=0;autoAnnotateOnImport();document.getElementById('dropzone').style.display='none';document.getElementById('chart-editor').style.display='block';_isDirty=false;render();}catch(err){console.error(err);alert(t('alertBadXML'));}});
['chart-title','meta-key','meta-tempo','meta-time','meta-style'].forEach(id=>{document.getElementById(id).addEventListener('change',e=>{const m={'chart-title':'title','meta-key':'key','meta-tempo':'tempo','meta-time':'timeSig','meta-style':'style'};chartData[m[id]]=e.target.value;if(id==='meta-key')originalKey=e.target.value;_isDirty=true;});});
